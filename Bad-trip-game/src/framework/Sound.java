...
public class Sound {
    ...
    public Sound(String f) {
        try {
            ...
            released = true;
        } catch(IOException | UnsupportedAudioFileException | LineUnavailableException | IllegalArgumentException exc) {
            exc.printStackTrace();
            released = false;
        }
    }
	...
	*/
    ...
    public static Sound playSound(String s, float x) {
        //File f = new File(s);
        if (x<0) x = 0;
        if (x>1) x = 1;
        float min = volumeC.getMinimum();
        float max = volumeC.getMaximum();
        System.out.println(volumeC.getMaximum());
        volumeC.setValue((max-min)*x+min);
        Sound snd = new Sound(s);
        snd.play(true);
        return snd;
